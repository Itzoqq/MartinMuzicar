...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.listening, name="!play | music"))
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.CommandNotFound):
        ...
        await ctx.send(f"Missing required argument: `{error.param.name}`. Use `!help {ctx.command}` for details.")
